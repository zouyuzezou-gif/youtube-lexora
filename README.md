...
- 获取字幕后暂停视频，按批次提前翻译整段内容；完成后自行播放，字幕随进度跨页高亮。
...
这是开发者模式安装的早期版本，尚未上架 Chrome Web Store。主要面向普通 `/watch?v=` 视频；没有原生字幕的视频不自动转写。字幕缓存保留最近 12 个视频，笔记最多 500 条。准备翻译期间请保持侧栏打开；停止时已发送的当前批次可能仍会完成并计费。
